...
export const modules = {
    weapon: {
        ...
        commands: {
            start: (state) => { state.weapon.current_state = 'started'; state.weapon.cooldown_timer = modules.weapon.base_cooldown; return state; },
            stop: (state) => { state.weapon.current_state = 'stopped'; return state; }
        },
        base_cooldown: 42,
        onFrame: (state) => {
    ...
    repairer: {
        ...
        onClick: (state) => {
            if (state.repairer.current_state === 'stopped') {
                ...
                state.repairer.next_command = state.repairer.next_command === 'start' ? 'stop' : 'start';
            }
            return state;
        },
        commands: {
            start: (state) => { state.repairer.current_state = 'started'; state.repairer.cooldown_timer = modules.repairer.base_cooldown; return state; },
            stop: (state) => { state.repairer.current_state = 'stopped'; return state; }
        },
        base_cooldown: 66,
        onFrame: (state) => {
...
export const genModuleState = (name) => {
    ...
    return {current_state: 'stopped', next_command: 'stop', cooldown: module.base_cooldown, cooldown_timer: 0};
};